...
def _develop_pick(sel, obs, db, target=2, max_turn=2):
    ...
    deck to the bench) or bench a Basic from hand. Fixes the lone-basic OPENING brick
    that loses games to aggro (real ladder losses 81957650/81957151). Gated to the
    opening so it does NOT cost mid/late-game tempo in the mirror (mirror-neutral at
    n=1200; +5.4pt vs Mega Lucario ex). Returns an option-index list or None."""
    from cg.api import SelectType, OptionType
    if sel.type != SelectType.MAIN or obs.current is None:
        return None
    st = obs.current
...
def _choose_improved_dev(sel, obs, db):
    """Improved (lethal-KO) + EARLY BOARD DEVELOPMENT (opening only). Shipped pilot."""
...
class SearchTeacher:
    ...
    def __call__(self, obs_dict: dict) -> list[int]:
        ...
        # in play) in the OPENING, ALWAYS develop -- play Buddy-Buddy Poffin / bench a
        # Basic -- instead of searching. Fixes the #1 real-loss cause (lone-basic brick;
        # +5.4pt vs Mega Lucario ex) at no mirror cost.
        if self.rollout_policy == "improved_dev":
            dev = _develop_pick(sel, obs, get_card_db(), target=2)
            if dev is not None:
