...
class SearchTeacher:
    def __init__(self, deck=None, rng=None, plies=1, opp_model=None, samples=1,
        ...
        self.plies = 2 if plies and plies >= 2 else 1
        self.rollout_depth = ROLLOUT_DEPTH_2PLY if self.plies == 2 else ROLLOUT_DEPTH
        # The opponent model only changes the simulated opponent turn, so it is
        # only meaningful at 2-ply; keep it None at 1-ply to preserve behaviour.
